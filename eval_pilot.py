...
def condition_only(data):
    ...
    for item in data:
        if item[3]: # correctness
            drive_correct +=1
            if item[5] == True: # conditioncorrectness
                distract_correct += 1
...
def video_item(data): # individual driving videos
    ...
    # else:
    #     keep = True
    return[(drive_con_correct,drive_no_con_correct,drive_con_incorrect,drive_no_con_incorrect),(distr_correct,distr_incorrect),(drive_c_distr_c,drive_c_distr_i,drive_i_distr_c,drive_i_distr_i)]
    ...
    for item in data:
        if item[5]:
            ...
            if item[3]:
                ...
                distr_i_driving_i += 1
    return ((correct,incorrect),(distr_c_driving_c,distr_i_driving_c,distr_c_driving_i,distr_i_driving_i))
...
def first_metric(data):
    ...
    for item in data:
        try:
            if item[2] != "" and not item[3]==1:
                result += 1
            elif item[2] != "" and item[3]==1:
                result -= 1
            elif item[2] == "" and item[3]==1:
                comp += 1
            else:
                comp -= 1
        except IndexError: # value will be some letter in this case
            continue
    return result - comp
...
def second_metric(data):
    ...
    for item in data:
        try:
            if item[2] != "":
                if not item[3]==1 and item[5]==1:
                    result += 1
                elif item[3]==1 and not item[5]==1:
                    result -= 1
        except IndexError:
            continue
    return result
...
discard = ["videos/Yqe62JAmjMI_7_31.mp4","videos/Yqe62JAmjMI_53_50.5.mp4","videos/GYhPr8RyvZU_16_34.mp4","videos/GYhPr8RyvZU_19_37.5.mp4"] # videos that were found to never be answered correctly
...
data["SubjectNumber"] = subject_number
data["Age"] = age
data["Year"] = year
data["Distractions"] = distractions
data["Experience"] = experience

participants = per_participant(discard,data)
condition = per_condition(discard,data)
video = per_video(data)
distraction_form = per_distraction_type(discard,data)
age = by_age(discard,data)
experience = by_experience(discard,data)
year = by_year(discard,data)
distraction = common_distraction(discard,data)
distract_item = get_distraction_item(discard,data)
results = dict()
...
for key in video:
    ...
    current = video_item(video[key]) # ((drive),(distract),(both))
    results["Video " + key]["drive"]["c"] = current[0][0]
    results["Video " + key]["drive"]["i"] = current[0][1]
    results["Video " + key]["distract"]["c"] = current[1][0]
    results["Video " + key]["distract"]["i"] = current [1][1]
    results["Video " + key]["both"]["cc"] = current[2][0]
    results["Video " + key]["both"]["ci"] = current[2][1]
    results["Video " + key]["both"]["ic"] = current[2][2]
    results["Video " + key]["both"]["ii"] = current[2][3]
...
for key in distract_item.keys():
    if key != None:
        ...
        (drive,both) = distraction_sample(distract_item[key]) # (drive,both)
...
for key in age.keys():
    ...
    results["Age " + key]["drive"]["c"] = drive[0]/amount
    results["Age " + key]["drive"]["i"] = drive[1]/amount
    results["Age " + key]["distract"]["c"] = (both[0] + both[2])/amount
    results["Age " + key]["distract"]["i"] = (both[1] + both[3])/amount
    results["Age " + key]["both"]["cc"] = both[0]/amount
    results["Age " + key]["both"]["ci"] = both[1]/amount
    results["Age " + key]["both"]["ic"] = both[2]/amount
    results["Age " + key]["both"]["ii"] = both[3]/amount
...
for key in year.keys():
    ...
    results["Year " + key]["drive"]["c"] = drive[0]/amount # get mean
    results["Year " + key]["drive"]["i"] = drive[1]/amount
    results["Year " + key]["distract"]["c"] = (both[0] + both[2])/amount
    results["Year " + key]["distract"]["i"] = (both[1] + both[3])/amount
    results["Year " + key]["both"]["cc"] = both[0]/amount
    results["Year " + key]["both"]["ci"] = both[1]/amount
    results["Year " + key]["both"]["ic"] = both[2]/amount
    results["Year " + key]["both"]["ii"] = both[3]/amount
...
for key in experience.keys():
    ...
    results["Experience " + key]["drive"]["c"] = drive[0]/amount
    results["Experience " + key]["drive"]["i"] = drive[1]/amount
    results["Experience " + key]["distract"]["c"] = (both[0] + both[2])/amount
    results["Experience " + key]["distract"]["i"] = (both[1] + both[3])/amount
    results["Experience " + key]["both"]["cc"] = both[0]/amount
    results["Experience " + key]["both"]["ci"] = both[1]/amount
    results["Experience " + key]["both"]["ic"] = both[2]/amount
    results["Experience " + key]["both"]["ii"] = both[3]/amount
...
for key in distraction.keys():
    if type(key) == str: # avoid nan
        ...
        results["PersDis " + key]["drive"]["c"] = drive[0]/amount
        results["PersDis " + key]["drive"]["i"] = drive[1]/amount
        results["PersDis " + key]["distract"]["c"] = (both[0] + both[2])/amount
        results["PersDis " + key]["distract"]["i"] = (both[1] + both[3])/amount
        results["PersDis " + key]["both"]["cc"] = both[0]/amount
        results["PersDis " + key]["both"]["ci"] = both[1]/amount
        results["PersDis " + key]["both"]["ic"] = both[2]/amount
        results["PersDis " + key]["both"]["ii"] = both[3]/amount
